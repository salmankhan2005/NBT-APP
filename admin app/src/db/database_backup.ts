...
export const normalizeImageUrl = (url?: string | null): string | undefined => {
  ...
  if (Platform.OS === 'android' && cleaned.includes('localhost:3001')) {
    cleaned = cleaned.replace('localhost:3001', '10.0.2.2:3001');
  }
  if (Platform.OS !== 'android' && cleaned.includes('10.0.2.2:3001')) {
...
class AdminDatabase {
  ...
  async getTrips(): Promise<Trip[]> {
    // ── Fix: if session not loaded yet, wait for it before fetching ──────────
    if (!this.token) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(apiTrips)) {
            // Map backend trips to Admin App Trip format
            ...
            // Merge: update or append to mockTrips
